 ...
 * @see {@link https://en.wikipedia.org/wiki/List_of_HTTP_status_codes List of HTTP status codes}
 */
export enum HttpStatusCode {
   ...
   * The request might or might not be eventually acted upon, and may be disallowed when processing
   * occurs.
   */
  Accepted = 202,

  /**
   * The server is a transforming proxy that received a 200 OK from its origin, but is returning a
   ...
   * behaviors. However, some Web applications and frameworks use the 302 status code as if it
   * were the 303.
   */
  Found = 302,
   ...
   * the response. Many HTTP clients (such as Mozilla and Internet Explorer) do not correctly handle
   ...
   * authentication and Digest access authentication. 401 semantically means "unauthenticated"
   ...
   * being encoded as a query-string of a GET request. Occasionally seen when a client has
   * improperly converted a POST request to a GET request with long query information, when the
   * client erroneously repeats the request with a long query string when it should be using POST,
